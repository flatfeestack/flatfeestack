...
$$
SELECT repo_id,
       (get_monthly_sponsor_amount(user_id) / days_in_month($1)) * 1000 /
       (countsponsoredrepos_at(user_id, $1))::FLOAT as sponsor_amounts
...
$$
SELECT repo_id,
       (get_monthly_sponsor_amount(user_id) / days_in_month(now()::date)) * 1000 /
       (countsponsoredrepos(user_id))::FLOAT as sponsor_amounts
FROM sponsored_repos
$$;
