...
var SupportedCurrencies = map[string]Currencies{
	"ETH": {Name: "Ethereum", Short: "ETH", Smallest: "wei", FactorPow: 18, IsCrypto: true, PayoutName: "eth"},
	"GAS": {Name: "Neo Gas", Short: "GAS", Smallest: "mGAS", FactorPow: 8, IsCrypto: true, PayoutName: "neo"},
...
func ParseTemplate(filename string, other map[string]string) string {
	textMessage := ""
	tmplPlain, err := template.ParseFiles("templates/" + filename)
	if err == nil {
