        ...
        <div class="auth-button-wrapper center-flex">
            {#if appState.user.id}
                <div transition:fade class="button-overlay">
                    <button class="button1 center p-050" onclick={() => goto("/user/search")}>Dashboard</button>
                </div>
            {:else}
                <div transition:fade class="button-overlay">
                    <button class="button1 center p-050" onclick={() => goto("/login")}>Login{#if loading}<Dots></Dots>{/if}</button>
                </div>
            {/if}
        </div>
