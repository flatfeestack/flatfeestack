<script lang="ts">
  ...
  import {getColor1} from "../../utils.ts";
  ...
  $: {
    const tmp = appState.trustedRepos.find((r: Repo) => {
      return r.uuid === repo.uuid;
    });
    const repoIsTrusted = tmp !== undefined;

    showUnTrustProgressBar = appState.reposToUnTrustAfterTimeout.some(
      (r) => r.uuid === repo.uuid
    );
  ...
  async function unTrust() {
    ...
    try {
      ...
      appState.trustedRepos = appState.trustedRepos.filter((r: Repo) => {
        return r.uuid !== repo.uuid;
      });
    } catch (e) {
      appState.setError(e);
    ...
    if ($reposToUnTrustAfterTimeout.some((r) => r.uuid === repo.uuid)) {
      ...
      try {
        if (!verifiedStar) {
          ...
          appState.trustedRepos = [...appState.trustedRepos, res];
        }
      } catch (e) {
        appState.setError(e);
      }
  ...
  <div class="container-col2 trust-icons-div">
    ...
    {#if repo.analyzed}
      <button
        ...
        class:disabled={appState.reposWaitingForNewAnalysis.length !== 0}
        disabled={appState.reposWaitingForNewAnalysis.length !== 0}
      >
        {repo.healthValue}
