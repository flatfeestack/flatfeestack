<script lang="ts">
  ...
  import HealthyRepoCard from "./HealthyRepoCard.svelte";
  import AdminSearchResult from "./AdminSearchResult.svelte";
  import ThresholdSettingsOverlay from "./ThresholdSettingsOverlay.svelte";
  ...
  onMount(async () => {
    if (!appState.loadedTrustedRepos) {
      try {
        appState.isSubmitting = true;
        appState.trustedRepos = await API.repos.getTrusted();
        appState.loadedTrustedRepos = true;
      } catch (e) {
        appState.setError(e);
      } finally {
...
<Main>
          ...
          <div class="container-small">
            ...
            <div class="container-small m-2 dropdown">
              <button class="button1 drop-button" id="drop-button"
                ><i class="fas fa-caret-up" /> {sortingTitle}</button
              >
          ...
          <div class="container-small">
            ...
            <div class="container-small m-2 dropdown">
              <button class="button1 drop-button" id="amount-drop-button"
                ><i class="fas fa-caret-up" /> {amountOfShownRepos}</button
              >
  ...
  >
    <button
      ...
      on:click={hideOverlay}>close <i class="fas fa-close" /></button
    >
